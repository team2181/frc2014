...
Feeder::Feeder() : Subsystem("Feeder") {
	...
	pneumaticDown = RobotMap::feederPneumaticDown;
	limitPassing = RobotMap::feederlimitPassing;
...
void Feeder::Down() {
	pneumaticDown->Set(true);
}
void Feeder::Passing() {
	while (!limitPassing->Get()){
		speedController->Set(0.8,0);
		Wait(0.005);
	}
	speedController->Set(0,0);
	Wait(0.005);
